...
//         "incidentDescription": "DescriptionDescriptionDescriptionDescriptionDescription",
//         "currentOdometerKm": 200000,
//         "status": "UNDER_REVIEW",
//         "policyStartDate": "2026-06-17",
//         "policyEndDate": "2027-06-17"
//     }
...
export default function ClaimList() {
    ...
    const fetchClaims = async () => {
        try {
            const res = await axios.get('http://localhost:8080/api/claim/getbyofficer', config);
            setClaims(res.data);
        } catch (error) {
                ...
                <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                        <thead className="bg-light">
                            <tr>
                                <th className="text-uppercase small fw-bold text-muted px-4 py-3" >Claim ID</th>
                                <th className="text-uppercase small fw-bold text-muted px-4 py-3" >Policy Ref</th>
                                <th className="text-uppercase small fw-bold text-muted px-4 py-3" >Vehicle No</th>
                        ...
                        <tbody>
                        {
                            claims.map((c,index) => (
                                    <tr key={index} className="border-bottom">
                                        ...
                                        <td className="px-4 py-3 small">{c.policyName}</td>
                                        <td className="px-4 py-3 small text-muted">{c.vehicleNumber}</td>
                                        <td className="px-4 py-3 small text-truncate" style={{ maxWidth: '260px' }}>{c.incidentDescription}</td>
                                        <td className="px-4 py-3 small text-muted">{ new Date(c.policyStartDate).toLocaleDateString()}</td>
